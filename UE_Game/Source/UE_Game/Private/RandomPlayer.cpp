	...
	GetWorld()->GetTimerManager().SetTimer(TimerHandle, [&]() 
	{
		...
		else if (GamemodeBase->IsGameStarted)
		{
			...
			if (DoesMove)
			{
				...
				//Selezioniamo una truppa randomica sul campo che sia ancora in vita e che possa muoversi
				do
				{
					SelectedTroop = Troops[FMath::RandRange(0, Troops.Num() - 1)];
				} while (!(SelectedTroop->GetHealth() > 0 && SelectedTroop->CanMove()));
			...
			{
				//Selezioniamo una truppa randomica sul campo che sia ancora in vita e che possa attaccare
				do
				{
					SelectedTroop = Troops[FMath::RandRange(0, Troops.Num() - 1)];
				} while (!(SelectedTroop->GetHealth() > 0 && SelectedTroop->CanAttack()));
